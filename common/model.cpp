	...
	if (file != NULL)
	{
		while (fgets(lineBuff, MAX_PARSER_CHARS, file) != NULL)
		{
			...
			else
			{
				...
				cout << "[ENGINE][ERROR] Undefined token." << endl;
				cnt++;
				continue;
			}
			cnt++;
		...
		clock_t end = clock();
		SDEBUG("File read took " << float(end - start) / CLOCKS_PER_SEC << " seconds.");

		//Rearange the vertices and normals
		//Print out results test

		SDEBUG("Vertex Count: " << vertexCount);
		SDEBUG("Normal Count: " << normalCount);
		SDEBUG("Triang Count: " << trianglesCount);
		SDEBUG("UVs    Count: " << textureCount);
...
unsigned int & Model::getTextureCount()
{
	return textureCount;
}
